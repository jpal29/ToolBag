...
def test_challenge(client):
	...
	response = client.post('/listening', 
...
def test_add_item(client):
	...
	response = client.post('/listening', 
								data=json.dumps(add_request.data), 
								headers=add_request.headers)
...
def test_remove_item(client):
	...
	response = client.post('/listening', 
								data=json.dumps(remove_request.data), 
...
def test_list_needed_items(client):
	...
	response = client.post('/listening', 
								data=json.dumps(list_request.data), 
...
def test_set_sass(client):
	set_sass_request = request_wrapper("set sass")

	response = client.post('/listening', 
								data=json.dumps(set_sass_request.data), 
								headers=set_sass_request.headers)
	
	
	assert "Sass is being set" in response.data
	assert response.status_code == 200
